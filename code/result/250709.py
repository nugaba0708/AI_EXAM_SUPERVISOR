''' 시선 상태가 캘리브레이션 완료 상태에서 실시간 업데이트 되지 않는 문제 해결'''
import cv2
            ...
            while True:
                ...
                if 'gaze_baseline' in status_data:
                    if current_gaze_abnormal:
                        direction = "왼쪽" if gaze_ratio < self.gaze_baseline else "오른쪽"
                        self.gaze_label.config(text=direction, fg="orange")
                    else:
                        self.gaze_label.config(text="정면", fg="green")
...
def speak_tts(text, lang='ko'):
    """개선된 threading을 사용한 비블로킹 TTS"""
    def tts_worker():
        try:
            import uuid
            unique_id = str(uuid.uuid4())[:8]
            tts_path = f"tts_output_{unique_id}.mp3"
